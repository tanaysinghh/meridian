...
> **Status.** The Blueprint validates and the services build cleanly. The
> current deployment of this repo is paused pending a plan decision on
> `meridian-db` — Render's free tier allows only one Postgres per account
> and this account already has one attached to another project. See
...
3. Cookies stay first-party per hostname (`app.co` and `api.app.co` each
   have their own auth cookie). Meridian doesn't currently emit a
   cross-subdomain cookie — if you need shared-apex cookies for a SPA on
   one host talking to an API on another, that's a small addition to the
   auth middleware.
